...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = 'تم تفعيل الحمايه لهاذه المجموعه\nيرجى رفع البوت اداري في المجموعه'
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = 'تم تعطيل الحمايه لهذه المجموعه 🛡\n👮 بامر من :️ @'..msg.from.username
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'yes' then
    return 'تم قفل الروابط سابقا في  المجموعه 😌\n👮 بامر من :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_link'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم قفل الروابط في  المجموعه بعد محد يدز 😌\n👮 بامر من :️ @'..msg.from.username
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return 'تم فتح الروابط سابقا في  المجموعه\n👮 بامر من :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_link'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح الروابط  في  المجموعه دزو براحتكم\n👮 بامر من :️ @'..msg.from.username
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'yes' then
    return 'التفليش مقفول سابقا ما تشوف محد يكدر يفلش😌\n👮 بامر من :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_spam'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم قفل التفلش بعد محد يكدر يفلش وانه موجود😌\n👮 بامر من :️ @'..msg.from.username
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return 'تم قفل التكرار سابقا ما شوف محد يفلش😌\n👮 بامر من :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['flood'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم قفل التكرار ما تشوف محد يدز اشياء بسرعه 😌\n👮 بامر من :️ @'..msg.from.username
  end
end
...
local function unlock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return 'تم فتح التكرار سابقا😌\n👮 بامر من :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['flood'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح التكرار ضلو كررو😜\n👮 بامر من :️ @'..msg.from.username
  end
end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return 'قفل العربيه مفعل سابقا ما تشوف محد يكتب عربي😂\n👮 بامر من :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم قفل الغه العربيه خل تتعلمون شوي الانكليزيه😄\n👮 بامر من :️ @'..msg.from.username
  end
end
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'تم فتح العربيه سبقا ما تشوف الكل يكتب عربي😒\n👮 بامر من :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح الغه العربيه كتبو براحتكم😌\n👮 بامر من :️ @'..msg.from.username
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'قفل الاضافه مفعل سابقا ما تشوف محد يضيف 😅\n👮 بامر من :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
    return 'تم قفل الاضافه خل اشوف واحد يضيف هسه😈\n👮 بامر من :️ @'..msg.from.username
end
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return 'تم فتح الاضافه مسبقا لتلح😠\n👮 بامر من :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح الاضافه ضيفو براحتكم😌✋\n👮 بامر من :️ @'..msg.from.username
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return 'قفل الملصقات مفعل سابقا ما تشوف محد يدز😌\n👮 بامر من :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم منع الملصقات خل اشوف واحد يدز بعد😎\n👮 بامر من :️ @'..msg.from.username
  end
end
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    return 'فتح الملصقات مفعل سابقا لتلح😒\n👮 بامر من :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح الملصقات دزو براحتكم ☺\n👮 بامر من :️ @'..msg.from.username
  end
end
...
local function lock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'yes' then
    return 'تم قفل جهات الاتصال سابقا ما تشوف ماكو ارقام بلكروب 🔐\n👮 بامر من :️ @'..msg.from.username
  else
    data[tostring(target)]['settings']['lock_contacts'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم قفل جهات الاتصال خل اشوف بعد منو يدز رقمه 🔐\n👮 بامر من :️ @'..msg.from.username
  end
end
...
local function get_rules(msg, data)
  ...
  if not data[tostring(msg.to.id)][data_cat] then
    return 'لم تقم بوضع قوانين للكروب \nلوضع قوانين اكتب : ضع القوانين'
  end
  local rules = data[tostring(msg.to.id)][data_cat]
  local group_name = data[tostring(msg.to.id)]['settings']['set_name']
  local rules = group_name..'قوانين الكروب ♨️\n\n'..rules:gsub("/n", " ")
  return rules
end
...
local function modlist(msg)
  ...
  if next(data[tostring(msg.to.id)]['moderators']) == nil then
    return '❌ لم تقم بترفيع ادمنيه\nلرفع ادمن في الكروب اكتب : رفع ادمن'
  end
  local i = 1
  local message = 'قائمه الادمنيه ' .. string.gsub(msg.to.print_name, '_', ' ') .. ':\n'
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'تفعيل' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, 'البوت بلفعل مفعل في الكروب سابقا ✔️..', ok_cb, false)
			end
		...
		if matches[1] == "اضهار البوتات" and is_momod(msg) then
			member_type = 'البوتات'
		...
		if matches[1] == 'ضع الرابط' and is_owner(msg) then
			data[tostring(msg.to.id)]['settings']['set_link'] = 'waiting'
			save_data(_config.moderation.data, data)
			return 'ارسل رابط كروبك هنا الان'
		end
		...
		if msg.text then
			if msg.text:match("^(https://telegram.me/joinchat/%S+)$") and data[tostring(msg.to.id)]['settings']['set_link'] == 'waiting' and is_owner(msg) then
				...
				save_data(_config.moderation.data, data)
				return "تم وضع رابط للكروب \nلعرض الرابط اكتب : الرابط"
			end
		end
		...
		if matches[1] == 'ضع الصوره' and is_momod(msg) then
		...
		if matches[1] == 'مسح' then
			...
			if matches[2] == 'الادمنيه' then
				if next(data[tostring(msg.to.id)]['moderators']) == nil then
					return 'لا توجد ادمنيه ليتم مسحهم ❌'
				end
				for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do
				...
				return 'تم مسح جميع ادمنيه البوت 🛡\n👮 بامر من :️ @'..msg.from.username
			end
		...
		if matches[1] == 'قفل' and is_momod(msg) then
			...
			local msg_type = 'Photo'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return 'تم قفل الصور بعد لحد يدز لان ينمسحن\n👮 الامر من :️ @'..msg.from.username
				else
					return 'تم قفل الصور سابقا\n👮 الامر من :️ @'..msg.from.username
				end
			end
